...
export function humanInstallError(detail: string): string {
  // 🔴 13.09: раньше неверный пароль уходил в общую ветку «разберусь — напишу» + «жми кнопку
  // снова», и человек жал её с тем же паролем, пока не бросал (журнал: 2 человека по 2–3 раза).
  if (/All configured authentication methods failed|Authentication failed/i.test(detail)) {
    return (
      'Сервер не принял root-пароль. Скорее всего, его сменили или на сервере переустановили систему ' +
      '(тогда пароль новый — он в панели хостинга или в письме).\n\n' +
      'Пройди настройку заново с актуальным паролем: /start → «Я уже купил сервер» → «Настроить».'
    );
  }
