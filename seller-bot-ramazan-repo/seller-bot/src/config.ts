...
export const config = {
  ...
  // Кому, кроме владельца, показывать /stats и /campaigns — БЕЗ доступа ко всему
  // остальному кабинету (цены, локации, отзыв ключей и т.п. остаются только владельцу).
  // Просьба 13.09 (Ramazan — партнёру kodeX нужна видимость аналитики, не управление).
  // Пусто по умолчанию — на всех узлах, кроме тех, где явно прописали в .env.
  statsViewerIds: new Set(
    (process.env.STATS_VIEWER_IDS ?? '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
  ),
